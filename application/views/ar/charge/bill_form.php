                                                                    ...
                                                                    foreach ($details->result() as $row_detail) {
                                                                        ...
                                                                        foreach ($qry_item->result() as $bill_item) {
                                                                            if($bill_item->itemstock_id == $row_detail->item_id){
                                                                                ...
                                                                                //$item = $bill_item->item_desc;
                                                                                $item = array('itemstock_id' => $bill_item->itemstock_id, 'item_code' => $bill_item->item_code, 'item_desc' => $bill_item->item_desc, 'price-lock' => $bill_item->price_lock, 'unit-price' => $bill_item->unit_price, 'unit-discount' => $bill_item->unit_discount, 'is-service' => $bill_item->is_service_item , 'max-qty' => $bill_item->itemstock_current_qty);
                                                                            }
                                                                        }
                                                                        ...
                                                                        echo '<tr>
                                                                            ...
                                                                            <input type="hidden" name="item_id[' . $i . ']" value="' . (count($item) > 0 ? $item['itemstock_id'] : '') . '">
                                                                            <input type="text" name="item_code[' . $i . ']" class="form-control input-sm text-center" value="' . (count($item) > 0 ? $item['item_code'] : '') . '" readonly />' .
                                                                            '<span class="input-group-btn">' .
                                                                            '<button class="btn btn-sm green-haze select_item" unique-id=""  data-index="' . $i . '" style="padding-top:5px;margin-right:0px;" type="button"><i class="fa fa-arrow-up fa-fw"></i></button>' .
                                                                            '</span></div>
                                                                            </td>
...
<script>
    $(document).ready(function(){
        ...
        var handleTableItem = function (itemId, index, uniqueId)   {
            var item_id_exist = '-';
            var n = 0;
            $('#table_extra tbody tr').each(function () {
                if ($(this).hasClass('hide') == false) {
                    var item_id = parseInt($(this).find('input[name*="item_id"]').val()) || 0;
                    if (item_id > 0) {
                        if(item_id_exist == '-'){
                            item_id_exist = '';
                        }
            ...
            grid_item.init({
                ...
                dataTable: {
                    "bStateSave": false, // save datatable state(pagination, sort, etc) in cookie.
                    "aoColumns": [
                        { "sClass": "text-center", "bSortable": true, "sWidth" : '12%' },
                        null,
                        { "sClass": "text-center","sWidth" : '10%' }, // TYPE
                        { "sClass": "text-center","sWidth" : '10%' }, // UOM
                    ...
                    "pageLength": 10, // default record count per page
                    "ajax": {
                        "url": "<?php echo base_url('pos/pos/get_item_list_by_id');?>/" + itemId + '/' + index + '/' + uniqueId + '/' + item_id_exist + '/'
                    }
                }
            });
        ...
        $('.select_item').live('click', function(e){
            e.preventDefault();
            ...
            var item_id = parseInt($(this).closest('td').find('input[name*="item_id"]').val()) || 0;
            ...
            setTimeout(function(){
                $modal.load('<?php echo base_url('pos/pos/ajax_modal_item');?>.tpd', '', function(){
                    handleTableItem(item_id, index, unique_id);
        ...
        $('.btn-select-item').live('click', function (e) {
            ...
            var item_id = parseInt($(this).attr('item-stock-id')) || 0;
            var i = $(this).attr('data-index');
            if (item_id > 0) {
                var item_code = $(this).attr('item-code');
                var item_desc = $(this).attr('item-desc');
                var price_lock = parseInt($(this).attr('price-lock')) || 0;
                var unit_price = parseFloat($(this).attr('unit-price')) || 0;
                var unit_discount = parseFloat($(this).attr('unit-discount')) || 0;
                var tax_rate = parseFloat($(this).attr('tax-type-percent')) || 0;
                var is_service = parseInt($(this).attr('is-service')) || 0;
                var unit_max_qty = $(this).attr('max-qty');
                var tax_code = $(this).attr('tax-type-code');
                var amount = unit_price - unit_discount;

                $('input[name="item_id[' + i + ']"]').val(item_id);
                $('input[name="item_code[' + i + ']"]').val(item_code);
                ...
                $('input[name="item_id[' + i + ']"]').val(0);
                $('input[name="item_code[' + i + ']"]').val('');
        ...
        function validate_submit(){
            ...
            $('#table_extra > tbody > tr ').each(function() {
                ...
                if(!$(this).hasClass('hide') && item_status == '<?php echo STATUS_NEW; ?>') {
                    if (result) {
                        var item_id = parseFloat($('input[name="item_id[' + i + ']"]').val()) || 0;
                        ...
                        if(item_qty <= 0){
                            ...
                            if(item_qty_max > 0){
                                if(item_qty > item_qty_max){
                                    toastr["error"]($('input[name="item_code[' + i + ']"]').val() + " Max qty is " + item_qty_max, "Warning");
                                    $('input[name="item_qty[' + i + ']"]').addClass('has-error');
                                    result = false;
                                }
                            }
