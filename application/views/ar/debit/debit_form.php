	...
	<div class="page-content">
		...
		<div class="page-bar">
			<ul class="page-breadcrumb">
                ...
                $form_mode = '';
                $back_url = base_url('ar/folio/debit/1.tpd');
                if($debitnote_id > 0){
                    ...
                    if($row->status == STATUS_CLOSED || $row->status == STATUS_POSTED){
                        $back_url = base_url('ar/folio/debit/2.tpd');
                    }
                }
				?>
			</ul>
                ...
                <div class="portlet box <?php echo BOX;?>">
					<div class="portlet-title">
						...
						<div class="actions">
                            ...
                            ?>
                            <a href="<?php echo (isset($row) ? $back_url : base_url('ar/folio/debit/1.tpd')); ?>" class="btn default green-stripe">
							...
							<div class="form-actions top">
                                <div class="row">
									<div class="col-md-8">
                                        ...
                                        ?>
                                                &nbsp;
                                                <button type="button" class="btn btn-sm blue btn-circle" id="posting-button" onclick="posting_record(<?php echo $debitnote_id; ?>);"><i class="fa fa-save"></i>&nbsp;Posting</button>
                                        <?php
                                            }
                                        }
                                        ?>
										...
										<div class="form-group">
                                            ...
                                            <div class="col-md-8">
                                                <div class="input-group">
                                                    <input type="hidden" name="reservation_id" value="<?php echo ($debitnote_id > 0 ? $row->reservation_id : '');?>">
                                                    <input type="text" class="form-control" name="guest_name" value="<?php echo ($debitnote_id > 0 ? $row->reservation_code . ' / ' . $row->tenant_fullname : '');?>" readonly />
                                                     <span class="input-group-btn">
...
<script>
    ...
    $(document).ready(function(){
        ...
        //COA
        var handleTableRsvt = function (num_index, reservation_id, debitnote_id) {
            ...
            grid_rsvt.init({
                ...
                dataTable: {
                    ...
                    "ajax": {
                        "url": "<?php echo base_url('ar/debit/get_modal_reservation');?>/" + num_index + "/" + reservation_id + "/" + debitnote_id // ajax source
                    }
                }
            });
        ...
        $('#btn_lookup_rsvt').on('click', function(){
            ...
            setTimeout(function () {
                $modal.load('<?php echo base_url('ar/debit/ajax_unallocated_reservation');?>.tpd', '', function () {
                    $modal.modal();

                    var debitnote_id = $('input[name="debitnote_id"]').val();
                    var rsvt_id = $('input[name="reservation_id"]').val();
                    handleTableRsvt(num_index, rsvt_id, debitnote_id);
        ...
        $('.btn-select-reservation').live('click', function (e) {
            ...
            $('input[name="guest_name"]').val(reservation_code + ' / ' + tenant_name);
        ...
        $('button[name="save"]').on('click', function(e){
            ...
            if($("#form-entry").valid()){
                var url = '<?php echo base_url('ar/debit/submit_debit.tpd');?>';
                    ...
                    callback: function(result) {
                        if(result === false){
                            ...
                            $.ajax({
                                ...
                                dataType: "json",
                                data: { debitnote_id: debitnote_id, posting_date : posting_date}
                            })
                                .done(function( msg ) {
                                    Metronic.unblockUI();
