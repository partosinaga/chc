	...
	<div class="page-content">
		...
		<div class="page-bar">
			<ul class="page-breadcrumb">
                ...
                $form_mode = '';
                $back_url = base_url('ar/corporate_bill/refund/1.tpd');
                if($refund_id > 0){
                    ...
                    if($row->status == STATUS_CLOSED || $row->status == STATUS_POSTED){
                        $back_url = base_url('ar/corporate_bill/refund/2.tpd');
                    }
                }
				?>
			</ul>
			...
			<div class="col-md-12">
                <div class="portlet box <?php echo BOX;?>">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-user"></i><?php echo ($refund_id > 0 ? '' : 'New');?> Official Receipt Refund
						</div>
						<div class="actions">
                            <?php
                            if($refund_id > 0){
                            ...
                            ?>
                            <a href="<?php echo (isset($row) ? $back_url : base_url('ar/corporate_bill/refund/1.tpd')); ?>" class="btn default green-stripe">
							...
							<div class="form-actions top">
                                <div class="row">
									<div class="col-md-8">
										...
										<button type="button" class="btn btn-sm blue-madison btn-circle hide" name="save_close" id ="btn_save_close">Save & Close</button>
                                        ...
                                        ?>
                                                &nbsp;
                                                <button type="button" class="btn btn-sm blue btn-circle" id="posting-button" onclick="posting_record(<?php echo $refund_id; ?>);"><i class="fa fa-check"></i>&nbsp;Posting</button>
                                        <?php
                                            }
                                        }
                                        ?>
								...
								<div class="row">
									...
									<div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-8">
                                                <div class="input-group">
                                                    <input type="hidden" name="receipt_id" value="<?php echo ($refund_id > 0 ? $row->receipt_id : '');?>">
                                                    <input type="hidden" name="company_id" value="<?php echo ($refund_id > 0 ? $row->company_id : '');?>">
                                                    <input type="hidden" name="reservation_id" value="0">
                                                    <input type="text" class="form-control" name="receipt_no" value="<?php echo ($refund_id > 0 ? $row->receipt_no : '');?>" readonly />
                                                     <span class="input-group-btn">
                                                       <a id="btn_lookup_receipt" class="btn btn-success" href="javascript:;" >
                                        ...
                                        <div class="form-group">
                                            ...
                                            <span class="col-md-6" id="label_company_name"><?php echo ($refund_id > 0 ? $row->company_name : '');?></span>
                                        </div>
                                        <div class="form-group">
...
<script>
    ...
    $(document).ready(function(){
        ...
        var handleTableReceipt = function (num_index, receipt_id, debitnote_id) {
            ...
            grid_comp.init({
                ...
                dataTable: {
                    ...
                    "ajax": {
                        "url": "<?php echo base_url('ar/refund/get_modal_unallocated_corp');?>/" + num_index + "/" + receipt_id + "/" + debitnote_id // ajax source
                    }
                }
            });
        ...
        $('#btn_lookup_receipt').on('click', function(){
            ...
            setTimeout(function () {
                $modal.load('<?php echo base_url('ar/refund/ajax_unallocated_corporate');?>.tpd', '', function () {
                    $modal.modal();

                    var refund_id = $('input[name="refund_id"]').val();
                    var receipt_id = $('input[name="receipt_id"]').val();
                    handleTableReceipt(num_index, receipt_id, refund_id);
        ...
        $('.btn-select-receipt').live('click', function (e) {
            ...
            $('#label_company_name').html(company_name);
        ...
        $('button[name="save"]').click(function(e){
            ...
            if(validate_input()){
                var url = '<?php echo base_url('ar/refund/submit_corp_refund.tpd');?>';
                    ...
                    callback: function(result) {
                        if(result === false){
                            ...
                            $.ajax({
                                ...
                                dataType: "json",
                                data: { refund_id: refund_id, posting_date : posting_date, is_corporate: 1}
                            })
                                .done(function( msg ) {
                                    Metronic.unblockUI();
