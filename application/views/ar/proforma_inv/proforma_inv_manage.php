			...
			<div class="col-md-12">
				...
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
                            <i></i>&nbsp;Proforma Invoice
                            <div class="btn-group btn-group-devided">
                                <a href="<?php echo base_url('ar/corporate_bill/proforma/1.tpd');?>" class="btn btn-transparent grey-gallery btn-circle btn-sm <?php echo $is_history ? '' : 'active'?>">Manage</a>
                                <a href="<?php echo base_url('ar/corporate_bill/proforma/2.tpd');?>" class="btn btn-transparent grey-gallery btn-circle btn-sm <?php echo $is_history ? 'active' : ''?>">History</a>
                            </div>
						</div>
						<div class="actions">
                            <?php
                                if(!$is_history && check_session_action(get_menu_id(), STATUS_APPROVE)){
                                    echo '<a style="margin-left:20px;" href="javascript:;" class="btn-primary btn blue btn-circle" id="approve-button"><i class="fa fa-save"></i>&nbsp;Approve</a>&nbsp;';
                                }
                                if(check_session_action(get_menu_id(), STATUS_NEW)) {
                                    //ORDER_STATUS::CHECKIN
                                    echo '<a href="' . base_url('ar/corporate_bill/proforma/3/0.tpd') . '" class="btn default yellow-stripe">
                                    <i class="fa fa-plus"></i>
                                    <span class="hidden-480"> New Proforma Invoice </span>
                                    </a>';
                                }
							?>
						</div>
					...
					<div class="portlet-body table-responsive">
						<div class="table-container">
                            ...
                            <table class="table table-striped table-bordered table-hover dataTable form-entry table-po-detail" id="table_pro_inv_manage">
                                <thead>
	...
	$(document).ready(function(){
		...
		var handleRecords = function () {
			...
			grid.init({
				...
				dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
					...
					"ajax": {
						"url": "<?php echo base_url('ar/proforma_inv/get_pro_inv_manage/' . get_menu_id() . '/' . ($is_history ? '1' : '0'));?>" // ajax source
					}
				}
			});
			
			var tableWrapper = $("#table_pro_inv_manage_wrapper");
		...
		$('.btn-bootbox').live('click', function(){
            ...
            if (action == '<?php echo STATUS_CANCEL;?>') {
                bootbox.prompt({
                    ...
                    callback: function (result) {
                        if (result === null) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "<?php echo base_url('ar/proforma_inv/ajax_action');?>",
                                dataType: "json",
                                data: {pro_inv_id: id, action: action, reason: result}
                            })
                                .done(function (msg) {
                                    Metronic.unblockUI();
                ...
                bootbox.confirm("Are you sure want to Approve " + code + " ?", function (result) {
                    if (result == true) {
                        Metronic.blockUI({
                            boxed: true
                        });
                        ...
                        $.ajax({
                            ...
                            url: "<?php echo base_url('ar/proforma_inv/ajax_action');?>",
                            dataType: "json",
                            data: {pro_inv_id: id, action: action}
                        })
                            .done(function (msg) {
                                Metronic.unblockUI();
                                console.log(msg);
        ...
        $("#table_pro_inv_manage #checkall").click(function () {
            if ($("#table_pro_inv_manage #checkall").is(':checked')) {
                $("#table_pro_inv_manage input[type=checkbox]").each(function () {
        ...
        $('#approve-button').click(function(e) {
            ...
            if (tot > 0) {
                bootbox.confirm("Are you sure want to Approve selected Porforma Invoice?", function (result) {
                    if (result == true) {
                        Metronic.blockUI({
                            target: '.form-entry',
                            boxed: true,
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?php echo base_url('ar/Porforma_inv/ajax_bill_multi_posting');?>",
                            dataType: "json",
                            data: form_data
                        })
                            .done(function (msg) {
