	...
	<div class="page-content">
		...
		<div class="page-bar">
			<ul class="page-breadcrumb">
                ...
                $form_mode = '';
                $back_url = base_url('ar/folio/deposit_al/1.tpd');
                if($allocationheader_id > 0){
                    if($row->status != STATUS_NEW){
                        $form_mode = 'disabled';
                        $back_url = base_url('ar/folio/deposit_al/2.tpd');
                    }
                }
				?>
			</ul>
			...
			<div class="col-md-12">
                <div class="portlet box <?php echo BOX;?>">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-user"></i><?php echo ($allocationheader_id > 0 ? '' : 'New');?> Deposit Folio Allocation
						</div>
						<div class="actions">
								...
								<div class="row">
									...
									<div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-8">
                                                <span id="company_name" class="control-label text-left bold"><?php echo ($allocationheader_id > 0 ? $row->reservation_code . ' - ' .$row->tenant_fullname : '');?></span>
                                            </div>
                                        </div>
                                        <?php
                                        $show_pending = true;
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-8">
                                                <div class="input-inline ">
                                                    <div class="input-group">
                                                        <span class="input-group-addon " style="font-size: 9pt;">IDR</span>
                                                        <input type="text" id="alloc_amount" name="alloc_amount" value="<?php echo ($allocationheader_id > 0 ? ($row->alloc_amount) : '0') ;?>" class="form-control text-right mask_currency input-medium font-red-sunglo" >
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function(){
            ...
            $.validator.addMethod("validateAlloc", function (value, element)
                {
                    ...
                    try{
                        ...
                        if(alloc_amount <= 0){
                            valid = false;
                            toastr["error"]("Alloc amount must not empty", "Warning");
                        }

                        if(alloc_amount > available_amount){
                            valid = false;
                            toastr["error"]("Alloc amount must not bigger than Available deposit", "Warning");
                        }

                        if(alloc_amount > unpaid_amount){
                            valid = false;
                            toastr["error"]("Alloc amount must not bigger than Unpaid amount", "Warning");
                        }
        ...
        var handleTableModal = function (num_index, depositdetail_id, allocationheader_id) {
            ...
            grid1.init({
                ...
                dataTable: {
                    ...
                    "ajax": {
                        "url": "<?php echo base_url('ar/deposit/get_modal_undeposit_folio');?>/" + num_index + "/" + depositdetail_id + "/" + allocationheader_id // ajax source
                    }
                }
            });
        ...
        $('#btn_lookup_company').on('click', function(){
            ...
            setTimeout(function () {
                $modal.load('<?php echo base_url('ar/deposit/xmodal_undeposit_folio');?>.tpd', '', function () {
                    $modal.modal();
        ...
        $('.btn-select-record').live('click', function (e) {
            ...
            $('#company_name').html(reservation_code + ' - ' + guest_name);
        ...
        $('button[name="save"]').click(function(e){
            ...
            if(validate_input()){
                var url = '<?php echo base_url('ar/deposit/submit_folio_al.tpd');?>';
                ...
                bootbox.confirm({
                    ...
                    callback: function(result) {
                        if(result === false){
                            ...
                            $.ajax({
                                type: "POST",
                                url: "<?php echo base_url('ar/deposit/xpost_folio_al');?>",
                                dataType: "json",
