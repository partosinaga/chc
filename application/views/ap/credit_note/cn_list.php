			...
			<div class="col-md-12">
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i>Credit Note <?php echo ($type == '0' ? 'Manage' : 'History');?>
						</div>
						<div class="actions">
							<?php
                                if(check_session_action(get_menu_id(), STATUS_NEW)){
                                    echo btn_new(base_url('ap/credit_note/cn_form/0.tpd'), 'New Credit Note');
                                }
                            ?>
						</div>
					</div>
							    ...
							    <table class="table table-striped table-bordered table-hover dataTable" id="datatable_ajax">
                                    <thead>
                                        <tr role="row" class="heading">
                                            ...
                                            <th width="70px" class="text-center"> CN Code </th>
                                            <th width="80px" class="text-center"> CN Date </th>
                                            <th width="130px" class="text-center"> Supplier </th>
                                            <th width="70px" class="text-center"> Inv. Code </th>
                                        ...
                                        <tr role="row" class="filter bg-grey-steel">
                                            <td>&nbsp;</td>
                                            <td>
                                                <input type="text" class="form-control form-filter input-sm" name="filter_cn_code">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-filter input-sm date date-picker margin-bottom-5" readonly name="filter_cn_date_from" placeholder="From" data-date-format="dd-mm-yyyy">
                                                <input type="text" class="form-control form-filter input-sm date date-picker" readonly name="filter_cn_date_to" placeholder="To" data-date-format="dd-mm-yyyy">
                                            </td>
                                            <td>
			...
			grid.init({
				...
				dataTable: {
					...
					"aoColumns": [
						...
						{ "sClass": "text-center" },                        //CN Code
						{ "sClass": "text-center" },                        //CN Date
                        null,                                               //Supplier
                        { "sClass": "text-center" },                        //Inv Code
					...
					"ajax": {
						"url": "<?php echo base_url('ap/credit_note/ajax_cn_list/' . get_menu_id() . '/' . $type);?>"
					},
                    ...
                    callback: function(result) {
                        if(result === null){ }
                            ...
                            $.ajax({
                                ...
                                dataType: "json",
                                data: {creditnote_id: data_id, action: action, reason:result }
                            })
                                .done(function( msg ) {
                                    Metronic.unblockUI();
                ...
                bootbox.confirm("Are you sure want to " + action_code + " " + data_code + " ?", function (result) {
                    if (result == true) {
                        ...
                        $.ajax({
                            ...
                            url: "<?php echo base_url('ap/credit_note/ajax_cn_action');?>",
                            dataType: "json",
                            data: {creditnote_id: data_id, action: action}
                        })
                            .done(function (msg) {
                                Metronic.unblockUI();
                                console.log(msg);
