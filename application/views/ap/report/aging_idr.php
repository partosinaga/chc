			...
			<div class="col-md-12" >
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i>Report Aging (IDR)
						</div>
					</div>
						...
						<div class="table-container table-responsive" >
                            <div class="col-md-12" style="padding-bottom: 20px; ">
                                <div class="table-actions-wrapper">
                                    <div class="input-group date date-picker date_select" data-date-format="dd-mm-yyyy">
...
<script>
	$(document).ready(function(){
		...
		var handleRecords = function () {
			grid.init({
				...
				dataTable: {
					...
					"ajax": {
						"url": "<?php echo base_url('ap/report/ajax_aging_idr/'. get_menu_id());?>"
					},
                    ...
                    "fnFooterCallback": function ( nRow, aaData, iStart, iEnd, aiDisplay ) {
                        ...
                        {
                            var d0_ = aaData[i][1].replace('<span class="mask_currency">', '');
                            var d0 = d0_.replace('</span>', '');
                            t_d0 += parseFloat((parseFloat(d0)).toFixed(2)) || 0;

                            var d31_ = aaData[i][2].replace('<span class="mask_currency">', '');
                            var d31 = d31_.replace('</span>', '');
                            t_d31 += parseFloat((parseFloat(d31)).toFixed(2)) || 0;

                            var d61_ = aaData[i][3].replace('<span class="mask_currency">', '');
                            var d61 = d61_.replace('</span>', '');
                            t_d61 += parseFloat((parseFloat(d61)).toFixed(2)) || 0;

                            var d91_ = aaData[i][4].replace('<span class="mask_currency">', '');
        ...
        $('.date_select').live('change', function(e){
