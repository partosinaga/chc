			...
			<div class="col-md-12">
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i>Debit Note <?php echo ($type == '0' ? 'Manage' : 'History');?>
						</div>
						<div class="actions">
							<?php
                                if(check_session_action(get_menu_id(), STATUS_NEW)){
                                    echo btn_new(base_url('ap/debit_note/dn_form/0.tpd'), 'New Debit Note');
                                }
                            ?>
						</div>
					</div>
							    ...
							    <table class="table table-striped table-bordered table-hover dataTable" id="datatable_ajax">
                                    <thead>
                                        <tr role="row" class="heading">
                                            ...
                                            <th width="70px" class="text-center"> DN Code </th>
                                            <th width="80px" class="text-center"> DN Date </th>
                                        ...
                                        <tr role="row" class="filter bg-grey-steel">
                                            ...
                                            <td>
                                                <input type="text" class="form-control form-filter input-sm date date-picker margin-bottom-5" readonly name="filter_dn_date_from" placeholder="From" data-date-format="dd-mm-yyyy">
                                                <input type="text" class="form-control form-filter input-sm date date-picker" readonly name="filter_dn_date_to" placeholder="To" data-date-format="dd-mm-yyyy">
                                            </td>
                                            <td>
			...
			grid.init({
				...
				dataTable: {
					...
					"aoColumns": [
						...
						{ "sClass": "text-center" },                        //DN Code
						{ "sClass": "text-center" },                        //DN Date
                        null,                                               //Supplier
					...
					"ajax": {
						"url": "<?php echo base_url('ap/debit_note/ajax_dn_list/' . get_menu_id() . '/' . $type);?>"
					},
                    ...
                    callback: function(result) {
                        if(result === null){ }
                            ...
                            $.ajax({
                                ...
                                dataType: "json",
                                data: {debitnote_id: data_id, action: action, reason:result }
                            })
                                .done(function( msg ) {
                                    Metronic.unblockUI();
                ...
                bootbox.confirm("Are you sure want to " + action_code + " " + data_code + " ?", function (result) {
                    if (result == true) {
                        ...
                        $.ajax({
                            ...
                            url: "<?php echo base_url('ap/debit_note/ajax_dn_action');?>",
                            dataType: "json",
                            data: {debitnote_id: data_id, action: action}
                        })
                            .done(function (msg) {
                                Metronic.unblockUI();
                                console.log(msg);
