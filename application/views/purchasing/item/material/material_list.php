			...
			<div class="col-md-12">
				...
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i>Meterial material List
						</div>
						<div class="actions">
							<?php if(check_session_action(get_menu_id(), STATUS_NEW)){ ?>
							<a href="<?php echo base_url('purchasing/item/material_manage/0.tpd');?>" class="btn default yellow-stripe">
							<i class="fa fa-plus"></i>
							<span class="hidden-480">
							New Item Material </span>
							</a>
							<?php } ?>
						</div>
					</div>
					<div class="portlet-body">
						<div class="table-container">
							...
							<tr role="row" class="filter bg-grey-steel">
								...
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_materialcode">									
								</td> 
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_materialdesc">
								</td>
								<td>
								...
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_factor">
								</td>
								<td>
									<select name="filter_uom_id_issue" class="form-control form-filter input-sm select2me">
										<option value="">All</option>
										<?php
											foreach($qry_uom->result() as $row_uom){
												echo '<option value="' . $row_uom->uom_id . '">' . $row_uom->uom_desc . '</option>';
								...
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_min_stock">
								</td>
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_max_stock">
								</td>
								<td>
	...
	$(document).ready(function(){
		var handleRecords = function () {
			...
			grid.init({
				...
				dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
					...
					"ajax": {
						"url": "<?php echo base_url('purchasing/item/material_list/' . get_menu_id());?>", // ajax source
					},
		...
		$('.btn-bootbox').live('click', function(){
			...
			bootbox.confirm("Are you sure?", function(result) {
				if(result == true){
					window.location.assign('<?php echo base_url('purchasing/item/material_delete');?>/' + id + '.tpd');
				}
			});
		});
