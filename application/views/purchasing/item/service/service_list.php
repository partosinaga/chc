			...
			<div class="col-md-12">
				...
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i>Item Service List
						</div>
						<div class="actions">
							<?php if(check_session_action(get_menu_id(), STATUS_NEW)){ ?>
							<a href="<?php echo base_url('purchasing/item/service_manage/0.tpd');?>" class="btn default yellow-stripe">
							<i class="fa fa-plus"></i>
							<span class="hidden-480">
							New Item Service </span>
							</a>
							<?php } ?>
						</div>
					</div>
					<div class="portlet-body">
						<div class="table-container">
							...
							<tr role="row" class="filter bg-grey-steel">
								...
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_servicecode">									
								</td> 
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_servicedesc">
								</td>
								<td>
	...
	$(document).ready(function(){
		var handleRecords = function () {
			...
			grid.init({
				...
				dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
					...
					"ajax": {
						"url": "<?php echo base_url('purchasing/item/service_list/' . get_menu_id());?>", // ajax source
					},
		...
		$('.btn-bootbox').live('click', function(){
			...
			bootbox.confirm("Are you sure?", function(result) {
				if(result == true){
					window.location.assign('<?php echo base_url('purchasing/item/service_delete');?>/' + id + '.tpd');
				}
			});
		});
