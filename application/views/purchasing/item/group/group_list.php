				...
				<div class="portlet">
					<div class="portlet-title">
						...
						<div class="actions">
							...
							<a href="<?php echo base_url('purchasing/item/group_manage/0.tpd');?>" class="btn default yellow-stripe">
					...
					<div class="portlet-body">
						<div class="table-container">
							...
							<tr role="row" class="filter bg-grey-steel">
								...
								<td>
									<select name="filter_class_id" class="form-control form-filter input-sm select2me">
										<option value="">All</option>
										<?php
											foreach($qry_class->result() as $row_class){
												echo '<option value="' . $row_class->class_id . '">' . $row_class->class_desc . '</option>';
											}
										?>
									</select>
								</td>
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_classcode">
								</td>
								<td>
									<input type="text" class="form-control form-filter input-sm" name="filter_classdesc">
								</td>
								<td>
									<select name="filter_department_id" class="form-control form-filter input-sm select2me">
										...
										<?php
											foreach($qry_department->result() as $row_dept){
												echo '<option value="' . $row_dept->department_id . '">' . $row_dept->department_desc . '</option>';
											}
										?>
									</select>
								</td>
	...
	$(document).ready(function(){
		var handleRecords = function () {
			...
			grid.init({
				...
				dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
					...
					"ajax": {
						"url": "<?php echo base_url('purchasing/item/group_list/' . get_menu_id());?>", // ajax source
					},
		...
		$('.btn-bootbox').live('click', function(){
			...
			bootbox.confirm("Are you sure?", function(result) {
				if(result == true){
					window.location.assign('<?php echo base_url('purchasing/item/group_delete');?>/' + id + '.tpd');
				}
			});
		});
