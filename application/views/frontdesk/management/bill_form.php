	...
	<div class="page-content">
		...
		<div class="page-bar">
			<ul class="page-breadcrumb">
                ...
                $form_mode = '';
                $isedit = true;
                $back_url = base_url('frontdesk/billing/bill_manage/1.tpd');
                if ($bill_id > 0) {
                    if ($bill->status != STATUS_NEW) {
                        $isedit = false;
                        $back_url = base_url('frontdesk/billing/bill_manage/2.tpd');
                    }
                }
			...
			<div class="col-md-12">
                <div class="portlet box <?php echo BOX;?>">
					<div class="portlet-title">
                        <div class="caption">
							<i class="fa fa-star"></i>Other Charges Form
						</div>
						<div class="actions">
                            <a href="<?php echo (isset($back_url) ? $back_url : base_url('frontdesk/billing/bill_manage/1.tpd')); ?>" class="btn default green-stripe">
                                                        ...
                                                        <div class="col-md-12">
                                                            ...
                                                            <table class="table table-striped table-hover table-bordered table-po-detail" id="table_extra" name="table_extra">
                                                                ...
                                                                if ($bill_id > 0) {
                                                                    ...
                                                                    $qry_item = $this->mdl_general->get('get_pos_item_active', array(), array(), 'item_desc');
                                                                    ...
                                                                    foreach ($details->result() as $row_detail) {
                                                                        ...
                                                                        foreach ($qry_item->result() as $row_item) {
                                                                            $item .= '<option value="' . $row_item->itemstock_id . '" data-price-lock="' . $row_item->price_lock . '" data-unit-price="' . $row_item->unit_price . '" data-unit-discount="' . $row_item->unit_discount . '" ' . ($row_item->itemstock_id == $row_detail->item_id ? 'selected="selected"' : '') . '>[ ' . $row_item->item_code . ' ] ' . $row_item->item_desc . '</option>';
                                                                        }
                                                                        ...
                                                                        echo '<tr>
                                                                            ...
                                                                            <td ><input type="text" name="item_rate[' . $i . ']" data-index="' . $i . '" value="' . $row_detail->rate . '" class="form-control text-right input-sm mask_currency num_cal" ' . ($row_item_detail->price_lock == '1' ? 'readonly' : '') . '></td>
    ...
    $(document).ready(function(){
        ...
        $('.charge_add').live('click', function (e) {
            ...
            if (reservationId > 0) {
                ...
                foreach($qry_item->result() as $row_item) {
                ?>
                item += '<option value="<?php echo $row_item->itemstock_id;?>" data-price-lock="<?php echo $row_item->price_lock;?>" data-unit-price="<?php echo $row_item->unit_price;?>" data-unit-discount="<?php echo $row_item->unit_discount;?>"><?php echo '[ ' . $row_item->item_code . ' ] ' . $row_item->item_desc;?></option>';
                <?php
                }
                ?>
        ...
        function validate_submit(){
            ...
            $('#table_extra > tbody > tr ').each(function() {
                if(!$(this).hasClass('hide')) {
                    ...
                    if (item_array.indexOf(item_id, 0) > -1) {
