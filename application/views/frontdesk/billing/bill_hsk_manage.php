			...
			<div class="col-md-12">
				...
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
                            <i></i>&nbsp;Housekeeping Charges
                            <div class="btn-group btn-group-devided">
                                <a href="<?php echo base_url('frontdesk/billing/bill_hsk_manage/1.tpd');?>" class="btn btn-transparent grey-gallery btn-circle btn-sm <?php echo($is_history ? '' : 'active'); ?>">Manage</a>
                                <a href="<?php echo base_url('frontdesk/billing/bill_hsk_manage/2.tpd');?>" class="btn btn-transparent grey-gallery btn-circle btn-sm <?php echo($is_history ? 'active' : ''); ?>">History</a>
                            </div>
						</div>
						<div class="actions">
                            <?php
                                ...
                                if(check_session_action(get_menu_id(), STATUS_NEW)) {
                                    ...
                                    echo '<a href="' . base_url('frontdesk/billing/bill_hsk_manage/3/0.tpd') . '" class="btn default yellow-stripe">
                                    <i class="fa fa-plus"></i>
                                    <span class="hidden-480"> Package </span>
                                    </a>';
                                }
							?>
						</div>
		...
		var handleRecords = function () {
			...
			grid.init({
				...
				dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
					...
					"ajax": {
						"url": "<?php echo base_url('frontdesk/billing/get_bill_hsk_manage/' . get_menu_id() . '/' . ($is_history ? '1' : '0'));?>" // ajax source
					}
				}
			});
            ...
            if (tot > 0) {
                bootbox.confirm("Are you sure want to Posting selected Billing ?", function (result) {
                    if (result == true) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?php echo base_url('frontdesk/billing/ajax_bill_multi_posting/1');?>",
                            dataType: "json",
                            data: form_data
                        })
                            .done(function (msg) {
