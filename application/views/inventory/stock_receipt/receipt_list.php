			...
			<div class="col-md-12">
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i>Stock Receipt <?php echo ($type == '0' ? 'Manage' : 'History');?>
						</div>
						<div class="actions">
							<?php if(check_session_action(get_menu_id(), STATUS_NEW)){ ?>
                                <a href="<?php echo base_url('inventory/stock_receipt/receipt_form/0.tpd');?>" class="btn default yellow-stripe">
                                    <i class="fa fa-plus"></i>
                                    <span class="hidden-480"> New Stock Receipt </span>
                                </a>
							<?php } ?>
						</div>
					</div>
							...
							<table class="table table-striped table-bordered table-hover dataTable" id="datatable_ajax">
                                <thead>
                                    <tr role="row" class="heading">
                                        ...
                                        <th width="12%" class="text-center"> Receipt Code </th>
                                        <th width="15%" class="text-center"> Receipt Date </th>
                                    ...
                                    <tr role="row" class="filter bg-grey-steel">
                                        <td>&nbsp;</td>
                                        <td>
                                            <input type="text" class="form-control form-filter input-sm" name="filter_sr_code">
                                        </td>
                                        <td>
                                            <div class="input-group date date-picker margin-bottom-5" data-date-format="dd-mm-yyyy">
                                                <input type="text" class="form-control form-filter input-sm" readonly name="filter_sr_date_from" placeholder="From">
                                                <span class="input-group-btn">
                                                <button class="btn btn-sm default" type="button"><i class="fa fa-calendar"></i></button>
                                                </span>
                                            </div>
                                            <div class="input-group date date-picker" data-date-format="dd-mm-yyyy">
                                                <input type="text" class="form-control form-filter input-sm" readonly name="filter_sr_date_to" placeholder="To">
                                                <span class="input-group-btn">
                                                <button class="btn btn-sm default" type="button"><i class="fa fa-calendar"></i></button>
                                                </span>
                                            </div>
		...
		var handleRecords = function () {
			...
			grid.init({
				...
				dataTable: {
					...
					"ajax": {
						"url": "<?php echo base_url('inventory/stock_receipt/ajax_receipt_list/' . get_menu_id() . '/' . $type);?>" // ajax source
					}
				}
			});
                    ...
                    callback: function(result) {
                        if(result === null){ }
                            ...
                            $.ajax({
                                ...
                                dataType: "json",
                                data: {sr_id: doc_id, action: action, reason:result }
                            })
                                .done(function( msg ) {
                                    Metronic.unblockUI();
                                    console.log(msg);
                ...
                bootbox.confirm("Are you sure want to " + action_code + " " + doc_code + " ?", function (result) {
                    if (result == true) {
                        ...
                        $.ajax({
                            ...
                            url: "<?php echo base_url('inventory/stock_receipt/ajax_action_receipt');?>",
                            dataType: "json",
                            data: {sr_id: doc_id, action: action}
                        })
                            .done(function (msg) {
                                Metronic.unblockUI();
                                console.log(msg);
