			...
			<div class="col-md-12">
				<div class="portlet">
					<div class="portlet-title">
						<div class="caption">
							<i class="fa fa-users"></i>GRN <?php echo ($type == '0' ? 'Manage' : 'History');?>
						</div>
						<div class="actions">
							<?php if(check_session_action(get_menu_id(), STATUS_NEW)){ ?>
                                <a href="<?php echo base_url('inventory/grn/grn_form/0.tpd');?>" class="btn default yellow-stripe">
                                    <i class="fa fa-plus"></i>
                                    <span class="hidden-480"> New GRN </span>
                                </a>
							<?php } ?>
						</div>
					</div>
							...
							<table class="table table-striped table-bordered table-hover dataTable" id="datatable_ajax">
                                <thead>
                                    <tr role="row" class="heading">
                                        <th width="20px"> # </th>
                                        <th width="80px"> GRN Code </th>
                                        <th width="100px"> GRN Date </th>
                                        <th width="80px"> PO Code </th>
                                    ...
                                    <tr role="row" class="filter bg-grey-steel">
                                        ...
                                        <td>
                                            <div class="text-center">
                                                <button class="btn btn-sm yellow filter-submit margin-bottom-5 tooltips" data-original-title="Filter" data-placement="top" data-container="body"><i class="fa fa-search"></i></button>
		...
		var handleRecords = function () {
			...
			grid.init({
				...
				dataTable: {
					...
					"ajax": {
						"url": "<?php echo base_url('inventory/grn/ajax_grn_list/' . get_menu_id() . '/' . $type);?>" // ajax source
					}
				}
			});
                    ...
                    callback: function(result) {
                        if(result === null){ }
                            ...
                            $.ajax({
                                ...
                                dataType: "json",
                                data: {grn_id: grn_id, action: action, reason:result }
                            })
                                .done(function( msg ) {
                                    Metronic.unblockUI();
                                    console.log(msg);
                ...
                bootbox.confirm("Are you sure want to " + action_code + " " + grn_code + " ?", function (result) {
                    if (result == true) {
                        ...
                        $.ajax({
                            ...
                            url: "<?php echo base_url('inventory/grn/ajax_grn_action');?>",
                            dataType: "json",
                            data: {grn_id: grn_id, action: action}
                        })
                            .done(function (msg) {
                                Metronic.unblockUI();
                                console.log(msg);
